...
+ Web version and BASH version of the comparitor script (Differ) that compares the log files generated by the indexer and downloader scripts.

# License

GNU General Public License v3.0

# Instructions: Web Versions
...
* Use Transfiler: Differ to compare index log files (or other log files created by these scripts) then manually analyse the success of the file transfer by reviewing the generated differences file.
...
1. Use this script to compare two index logs.
...
**Summary of Script Functions and Usage Instructions:**

*   **`compare_logs.php` (Web Script):**
    *   **Function:** Compares two CSV log files (uploaded through a web form) and generates a new CSV file containing the differences. It detects entries present in only one log, size differences, and type differences.
    *   **Usage:**
        1.  Copy the `compare_logs.php` file to a directory accessible by your web server.
        2.  Ensure that PHP is installed and configured to allow file uploads. You can set the configurations in `php.ini` (e.g., `upload_max_filesize`, `post_max_size`).
        3.  Access the script through a web browser.
        4.  Upload the two CSV log files (A and B) using the file input fields.
        5.  Click the "Compare Logs" button.
        6.  If successful, a link to the generated differences CSV file will be displayed.
*   **`compare_logs.sh` (BASH Script):**
    *   **Function:** Compares two CSV log files (provided as command-line arguments) and generates a new CSV file containing the differences. The script is designed to be run from the command line.
    *   **Usage:**
        1.  Save the script as `compare_logs.sh`.
        2.  Make the script executable: `chmod +x compare_logs.sh`.
        3.  Run the script from the command line, providing the paths to the two log files:

            ```bash
            ./compare_logs.sh log_file_a.csv log_file_b.csv
            ```

        4.  The differences will be written to a file named `log_differences_YYYYMMDD_HHMMSS.csv` in the same directory as the script.

With these two scripts, you now have the flexibility to compare file index logs either through a web interface or directly from the command line, depending on your needs. Remember to secure the web script properly in a production environment.
